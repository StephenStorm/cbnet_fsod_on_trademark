...
def cutout(img, pad_size, replace):
    ...
    low_h, high_h = torch.clamp(center_h - pad_size, 0, h-1).item(), torch.clamp(center_h + pad_size, 0, h-1).item()
    low_w, high_w = torch.clamp(center_w - pad_size, 0, w-1).item(), torch.clamp(center_w + pad_size, 0, w-1).item()
...
def bbox_cutout(img, bboxs, pad_fraction, replace_with_mean):
    ...
    x_min, x_max = int(torch.clamp(mask_x - mask_w, 0, w-1).item()), int(torch.clamp(mask_x + mask_w, 0, w-1).item())
    y_min, y_max = int(torch.clamp(mask_y - mask_h, 0, h-1).item()), int(torch.clamp(mask_y + mask_h, 0, h-1).item())
...
def _rotate_bbox(img, bboxs, degrees):
    ...
    for bbox in bboxs:
        ...
        rotate_min_x, rotate_max_x = torch.clamp(x_min + w / 2, 0, w-1), torch.clamp(x_max + w / 2, 0, w-1)
        rotate_min_y, rotate_max_y = torch.clamp(y_min + h / 2, 0, h-1), torch.clamp(y_max + h / 2, 0, h-1)
...
def translate_bbox(img, bboxs, pixels, replace, shift_horizontal):
    ...
    if shift_horizontal:
        for bbox in bboxs:
            ...
            translate_min_x, translate_max_x = torch.clamp(min_x+pixels, 0, w-1), torch.clamp(max_x+pixels, 0, w-1)
        ...
        for bbox in bboxs:
            ...
            translate_min_y, translate_max_y = torch.clamp(min_y+pixels, 0, h-1), torch.clamp(max_y+pixels, 0, h-1)
...
def shear_with_bboxes(img, bboxs, level, replace, shift_horizontal):
    ...
    if shift_horizontal:
        ...
        for bbox in bboxs:
            ...
            shear_min_x, shear_max_x = torch.clamp(x_min, 0, w-1), torch.clamp(x_max, 0, w-1)
            shear_min_y, shear_max_y = torch.clamp(y_min, 0, h-1), torch.clamp(y_max, 0, h-1)
...
def translate_only_bboxes(img, bboxs, p, pixels, replace, shift_horizontal):
    ...
    for bbox in bboxs:
        if torch.rand(1) < p:
            ...
            # print(img.shape, min_y, max_y+1, min_x, max_x+1)
